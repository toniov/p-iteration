...
/**
 * Same functionality as [`forEach()`](global.html#forEach), but only a maximum of `limit` callbacks are run concurrently.
 * @param {Array} array - Array to iterate over.
 * @param {numebr} limit - Number of concurrent iterations.
 * @param {Function} callback - Function to apply each item in `array`. Accepts three arguments: `currentValue`, `index` and `array`.
 * @param {Object} [thisArg] - Value to use as *this* when executing the `callback`.
 * @return {Promise} - Returns a Promise with undefined value.
 */
exports.forEachLimit = async (array, limit, callback, thisArg) => {
  let from = 0;
  let to = limit;
  const iterations = array.length / limit;
  for (let i = 0; i < iterations; i++) {
    const sliced = array.slice(from, to);
    await this.forEach(sliced, callback, thisArg);
    from += limit;
    to += limit;
  }
};
...
/**
 * Same functionality as [`map()`](global.html#map), but only a maximum of `limit` callbacks are run concurrently.
 * @param {Array} array - Array to iterate over.
 * @param {numebr} limit - Number of concurrent iterations.
 * @param {Function} callback - Function to apply each item in `array`. Accepts three arguments: `currentValue`, `index` and `array`.
 * @param {Object} [thisArg] - Value to use as *this* when executing the `callback`.
 * @return {Promise} - Returns a Promise with the resultant *Array* as value.
 */
exports.mapLimit = async (array, limit, callback, thisArg) => {
  let from = 0;
  let to = limit;
  const iterations = array.length / limit;
  const result = [];
  for (let i = 0; i < iterations; i++) {
    const sliced = array.slice(from, to);
    const iterationResult = await this.map(sliced, callback, thisArg);
...
/**
 * Same functionality as [`find()`](global.html#find), but only a maximum of `limit` callbacks are run concurrently.
 * @param {Array} array - Array to iterate over.
 * @param {numebr} limit - Number of concurrent iterations.
 * @param {Function} callback - Function to apply each item in `array`. Accepts three arguments: `currentValue`, `index` and `array`.
 * @param {Object} [thisArg] - Value to use as *this* when executing the `callback`.
 * @return {Promise} - Returns a Promise with the element that passed the test as value, otherwise *undefined*.
 */
exports.findLimit = async (array, limit, callback, thisArg) => {
  let from = 0;
  let to = limit;
  const iterations = array.length / limit;
  let result;
  for (let i = 0; i < iterations; i++) {
    ...
    const sliced = array.slice(from, to);
    result = await this.find(sliced, callback, thisArg);
    if (result) {
      break;
    }
    from += limit;
