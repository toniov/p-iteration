 ...
 * @param {number} [concurrency] - Number of max promise runing at the same time.
 * @return {Promise} - Returns a Promise with undefined value.
 */
exports.forEach = async (array, callback, thisArg, concurrency) => {
  const promiseArray = [];
  ...
  if (!concurrency) {
    ...
    const poolTask = pool.addEachTask({
      data: promiseArray,
      generator: (element) => {
        return element;
      }
    });
    await poolTask.promise();
  }
};
 ...
 */
exports.map = async (array, callback, thisArg, concurrency) => {
  const promiseArray = [];
  for (let i = 0; i < array.length; i++) {
    if (i in array) {
